...
**Verified Compliance:** This client is fully compliant with Aleatoric Engine API v0.5.6 and MCP Protocol Version 2024-11-05 (JSON-RPC 2.0). All examples are tested against live production endpoints.
...
**Expected output:**
```json
{"status": "ok", "version": "0.5.6", "timestamp": "2026-01-15T..."}
```
...
- **Health:** `curl -s -H "X-API-Key: $ALEATORIC_API_KEY" https://mcp.aleatoric.systems/mcp/health` → expect `{ "status": "ok", "version": "0.5.6", "timestamp": "..." }`.
